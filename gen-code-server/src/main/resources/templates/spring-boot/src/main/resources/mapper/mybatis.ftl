!#parse
!#name /src/main/resources/mapper/${classInfo.businessModule}/${classInfo.name}Mapper.xml
!#list
...
<mapper namespace="${classInfo.mapperPackage()}.${classInfo.name}Mapper">
    ...
    <#if classInfo.columnList?exists && classInfo.columnList?size gt 0>
        <#list classInfo.columnList as fieldItem >
            <result column="${fieldItem.tableColumn.columnName}" jdbcType="${fieldItem.tableColumn.type}" property="${fieldItem.beanAttr.name}" />
        </#list>
    </#if>
    </resultMap>
    ...
    <sql id="queryWhere">
        <where>
         <#if classInfo.columnList?exists && classInfo.columnList?size gt 0>
          <#list classInfo.columnList as fieldItem >
              <if test="${fieldItem.beanAttr.name} != null">
                  and `${fieldItem.tableColumn.columnName}` = ${r"#{"}${fieldItem.beanAttr.name}${r"}"}
              </if>
          </#list>
          </#if>
        </where>
    ...
    <select id="selectByPrimaryKey" resultMap="BaseResultMap">
        ...
        <include refid="Base_Column_List" />
        from ${classInfo.tableName}
        where ${classInfo.primaryKey.tableColumn.columnName} = ${r"#{"}${classInfo.primaryKey.beanAttr.name}${r"}"}
    </select>
    ...
    <#if classInfo.primaryKey?exists && classInfo.primaryKey.tableColumn.autoIncrement>
    ...
    <#if classInfo.columnList?exists && classInfo.columnList?size gt 0>
        ...
        <trim prefix="(" suffix=")" suffixOverrides=",">
                <#list classInfo.columnList as fieldItem >
                 <#if !fieldItem.tableColumn.autoIncrement>
                    ${r"#{item."}${fieldItem.beanAttr.name}${r"}"},
                 </#if>
                </#list>
            </trim>
        </foreach>
    ...
    <#if classInfo.columnList?exists && classInfo.columnList?size gt 0>
        ...
        <trim prefix="values (" suffix=")" suffixOverrides=",">
            <#list classInfo.columnList as fieldItem >
                <#if !fieldItem.tableColumn.autoIncrement>
                <if test="${fieldItem.beanAttr.name} != null">
                    ${r"#{"}${fieldItem.tableColumn.columnName}${r"}"},
                </if>
                </#if>
            </#list>
        </trim>
        ...
        <set>
            <#list classInfo.columnList as fieldItem >
                <#if !fieldItem.tableColumn.autoIncrement>
                <if test="${fieldItem.beanAttr.name} != null">
                    `${fieldItem.tableColumn.columnName}` = ${r"#{"}${fieldItem.beanAttr.name}${r"}"},
                </if>
                </#if>
            </#list>
        </set>
