!#parse
!#name /src/main/java/${classInfo.basePackage()}.service.impl/${classInfo.name}ServiceImpl.java
!#list
...
public class ${classInfo.name}ServiceImpl implements I${classInfo.name}Service {
	...
	* 删除
	*/
	public boolean delete(${classInfo.primaryKey.beanAttr.typeClassName} ${classInfo.primaryKey.beanAttr.name}){
		return ${classInfo.name?uncap_first}Mapper.deleteByPrimaryKey(${classInfo.primaryKey.beanAttr.name}) > 0;
	}
	...
	* 查询
	*/
	public ${classInfo.name}Dto seletBy${classInfo.primaryKey.beanAttr.name?cap_first}(${classInfo.primaryKey.beanAttr.typeClassName} ${classInfo.primaryKey.beanAttr.name}){
		${classInfo.name}Entity entity = ${classInfo.name?uncap_first}Mapper.selectByPrimaryKey(${classInfo.primaryKey.beanAttr.name});
		if(entity ==null){
        	return null;
		}
